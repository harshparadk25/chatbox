...
const Sidebar = ({ onSelectUser }) => {
    ...
    useEffect(() => {
        const chatUserHandler = async () => {
            setLoading(true)
            try {
                const chatters = await axios.get(`/api/user/currentChatter`)
        ...
        <div className='h-full w-auto px-1'>
            ...
            {searchUser?.length > 0 ? (
                ...
                <>
                    ...
                    <div className='mt-auto px-1 py-1 flex'>
                        <button onClick={handelLogOut} className='bg-white hover:bg-red-600 w-10 cursor-pointer hover:text-white rounded-lg'>
                            <CiLogout size={25} />
                        </button>
